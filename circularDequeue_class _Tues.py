...
class CircularDeque(CircularQueueOneSlotEmpty):
    ...
    def add_rear(self, item): # 후단에서 검색
    ...
    def add_front(self, item):
        if self.is_full():
            ...
            self.front = (self.front - 1) % self.N # front를 반시계 반향으로 한 칸 회전
    ...
    def delete_rear(self): # 후단에서 삭제
        if self.is_empty():
            ...
            item = self.array[self.rear] # rear 포인터는 마지막 요소을 가리킴
    ...
    def get_rear(self): # 후단에서 최근 자료 검색
        if self.is_empty():
            ...
            return self.array[self.rear] # 바로 rear위치
