...
def quick_sort(arr, left, right): # 퀴정렬
    #left, right는 정렬할 구간의 양 끝 인덱스
    def partition(A, left, right): # 분할 함수
        pivot = A[left] #pivotdms 시작 요소로 설정
        low = left + 1 # pivot다음부터 시작해서 pivot보다 작은 값 찾기
        high = right # pivot보다 큰 값 찾기

        while low <= high: # 엇갈릴 떄까지 반복
    ...
    if left < right: # # 정렬할 원소가 2개 이상인 경구
        q = partition(arr, left, right) # 피벗의 분할 위지 착고
...
def radix_sort(arr, bucket_size = 10): # 기수 정렬
    ...
    while max_val // factor > 0:
        for i in range(n): #배열의 모든 워소에 대해
        ...
        print(f"기수정렬 중간 단계 (factor = {factor}): {arr[:100]}...")
...
for n in sizes:
    ...
    data = []
    for _ in range(n):
        data.append(random.randint(1, 99999)) # 1 ~ 99999 사이에 있는 난수 정수 생성
